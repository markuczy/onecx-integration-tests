...
exports.importMicroservices = importMicroservices;
exports.importMicrofrontends = importMicrofrontends;
const tslib_1 = require("tslib");
const promises_1 = require("fs/promises");
const path_1 = tslib_1.__importDefault(require("path"));
const axios_1 = tslib_1.__importDefault(require("axios"));
const imports_logger_1 = require("../utils/imports-logger");
const logger = new imports_logger_1.Logger('ImportProductStore');
async function importProducts(baseDir, endpointBase) {
    logger.info('IMPORT_PRODUCTS_START');
    const dir = path_1.default.join(baseDir, 'products');
    ...
    for (const file of files) {
        ...
        const data = await (0, promises_1.readFile)(path_1.default.join(dir, file), 'utf-8');
...
async function importSlots(baseDir, endpointBase) {
    logger.info('IMPORT_SLOTS_START');
    const dir = path_1.default.join(baseDir, 'slots');
    const files = await (0, promises_1.readdir)(dir);
    for (const file of files) {
        if (!file.endsWith('.json'))
            continue;
        ...
        if (Array.isArray(parsedData) &&
            parsedData.every((entry) => entry && typeof entry === 'object' && typeof entry.appId === 'string' && Array.isArray(entry.slots))) {
            ...
            for (const entry of parsedData) {
                ...
                for (const slot of entry.slots) {
                    ...
                    try {
                        const response = await axios_1.default.put(endpoint, slot, {
                            headers: { 'Content-Type': 'application/json' },
                            validateStatus: () => true,
                        });
                        logger.status('UPLOAD_SUCCESS', response.status, `Slot ${slot.name} for app ${entry.appId} in product ${product}`);
                    }
                    catch (err) {
        ...
        try {
            const response = await axios_1.default.put(endpoint, parsedData, {
                headers: { 'Content-Type': 'application/json' },
                validateStatus: () => true,
            });
...
async function importMicroservices(baseDir, endpointBase) {
    logger.info('IMPORT_MICROSERVICES_START');
    const dir = path_1.default.join(baseDir, 'microservices');
    const files = await (0, promises_1.readdir)(dir);
    for (const file of files) {
        if (!file.endsWith('.json'))
            continue;
...
async function importMicrofrontends(baseDir, endpointBase, port) {
    logger.info('IMPORT_MICROFRONTENDS_START');
    const dir = path_1.default.join(baseDir, 'microfrontends');
    const files = await (0, promises_1.readdir)(dir);
    for (const file of files) {
        if (!file.endsWith('.json'))
            continue;
        ...
        try {
            const response = await axios_1.default.put(endpoint, mfeData, {
                headers: { 'Content-Type': 'application/json' },
                validateStatus: () => true,
            });
...
//# sourceMappingURL=import-product-store.js.map